...
HermiteSpline::HermiteSpline(const std::vector<vec3>& points, std::vector<vec3>& tangents, const CurveForm form):
    ...
    controlTangents(std::move(tangents)) {
    ComputeCentroid();
...
void HermiteSpline::ComputeCatmullRomTangents() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        controlTangents[i] = 0.5f * (controlPoints[next] - controlPoints[prev]);
    }
}
...
vec3 HermiteSpline::EvaluateSpline(const float t, const EvaluationType type) const {
    if (controlPoints.size() < 2 || controlTangents.size() < 2) {
    ...
    const vec3 m0 = controlTangents[i0];
    const vec3 m1 = controlTangents[i1];
    switch (type) {
