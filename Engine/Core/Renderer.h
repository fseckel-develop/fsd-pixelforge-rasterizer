...
#include <string>
class Scene; class Mesh; class LightNode; class RenderUnit;
...
public:
    ...
    static void Render(const shared_ptr<LightNode>&, RenderMode = FILLED);
...
private:
    ...
    static void SetLightingUniforms(const vector<shared_ptr<LightNode>>&);
    static void SetMaterialUniforms(const shared_ptr<Material>&);
    static void DrawMesh(const shared_ptr<Mesh>&);
    static void DrawLight(const shared_ptr<LightNode>&);
